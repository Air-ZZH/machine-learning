...
# ✅ 生成一张“手写图像”的模拟数据（随机值）
image = torch.rand(1, 1, 16, 16)  # [batch=1, channel=1, H=16, W=16]
...
# ✅ 前向传播（inference）
output = model(image)  # 输出 shape: [1, 10]
...
print(f"模型认为这张图片是数字：{predicted_class}")

image_2d = image.squeeze().numpy()

# ✅ 打印二维像素矩阵（保留两位小数）
np.set_printoptions(precision=2, suppress=True)
print("图像像素矩阵（灰度值 0~1）：")
print(image_2d)
...
output = torch.tensor([[5, 0.2, 2]])  # 模型输出的 logits
label = torch.tensor([0])  # 正确类别是第0类

loss_fn = nn.CrossEntropyLoss()
loss = loss_fn(output, label)

print(loss.item())  # 损失越大，说明模型越不对
'''
import torch # 1.导入库
import torch.nn as nn
